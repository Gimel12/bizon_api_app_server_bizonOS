 ...
 */
function localExec(command, timeoutMs = 30000, { asRoot = false } = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (command.trim().startsWith('sudo ')) {
      // Already privileged — strip sudo prefix and run directly (we're root)
      shellCmd = command.replace(/^sudo\s+/, '');
    } else if (asRoot) {
      // Privileged command — run directly as root
      shellCmd = command;
    } else {
